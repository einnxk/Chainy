...
/**
 * A {@code Chain} describes a workflow of steps that each consume the return value of the step
 * before them and produce the input for the step after them.
 *
 * <p><b>Sync vs. async is not a property of the builder.</b> A chain built with {@link #then(int,
 * ChainyFunction)} and one built with {@link #thenAsynchronously(int, AsyncChainyFunction)} produce
 * the exact same {@code Chain<R>} type and can be freely mixed. Every step, whether defined
 * synchronously or asynchronously, is capable of running in either mode. Which mode is actually
 * used is decided once, at the very end, by calling either {@link #blocking()}/{@link
 * #blocking(Duration)} or {@link #asynchronously()}/{@link #asynchronously(Executor)}. A step
 * originally defined with {@code then} that ends up inside an {@code asynchronously()} execution is
 * dispatched onto the executor; a step originally defined with {@code thenAsynchronously} that ends
 * up inside a {@code blocking()} execution is simply joined.
 *
...
public interface Chain<T> {
  ...
  static @NonNull <R> Chain<R> startWith(int stepCount, @NonNull ChainySupplier<R> supplier) {
    return SimpleChain.ofStep(Step.ofStart(stepCount, supplier));
  }
   ...
   * @return a new {@link Chain} builder
   */
  static @NonNull <R> Chain<R> startWithAsynchronously(
      int stepCount, @NonNull ChainySupplier<CompletableFuture<R>> supplier) {
    return SimpleChain.ofStep(Step.ofStartAsync(stepCount, supplier));
  }
  ...
  /**
   * Chains a synchronous step that consumes the current value {@code T} and produces a new value
   * {@code R}.
   *
   * @param stepCount the execution order assigned to this step
  ...
  /**
   * Chains an asynchronous step that consumes the current value {@code T} and produces a new value
   * {@code R}. Behaves identically to {@link #then(int, ChainyFunction)} once the chain is executed
   * — see the class-level documentation for how the execution mode is resolved.
   *
   * @param stepCount the execution order assigned to this step
   * @param function the function receiving the previous step's return value
   * @param <R> the type produced by this step
   * @return the same chain, now producing {@code R}
   */
  @NonNull <R> Chain<R> thenAsynchronously(
      int stepCount, @NonNull AsyncChainyFunction<T, R> function);

  /**
   * Passes the current value to a consumer without changing it, then continues the chain with that
   * same value.
   *
   * @param stepCount the execution order assigned to this step
  ...
  /**
   * Chains a step that is only applied if {@code predicate} holds for the current value; otherwise
  ...
  /**
   * Chains an if/else step: {@code ifFunction} is applied if {@code predicate} holds for the
   * current value, otherwise {@code elseFunction} is applied.
   *
   ...
   * @return the same chain, still producing {@code T}
   */
  @NonNull Chain<T> thenIf(
      @NonNull Predicate<T> predicate,
      @NonNull ChainyFunction<T, T> ifFunction,
      @NonNull ChainyFunction<T, T> elseFunction);

  /**
   * Waits the given duration before continuing with the next step. The current value is passed
   * through unchanged.
   *
  ...
  /**
   * Runs several asynchronous functions concurrently against the current value and collects their
   * results, in order, into a single list.
   *
  ...
  /**
   * Re-executes the whole chain from the start-up to {@code maxAttempts} times if a step throws,
   * waiting {@code delay} between attempts.
   *
  ...
  /**
   * Same as {@link #withRetry(int, Duration)}, but additionally consults {@code predicate} after
   * each failed attempt to decide whether another attempt should be made at all.
   *
   * @param maxAttempts the total number of attempts, including the first
   * @param delay the wait time between attempts
  ...
  /**
   * Fails the chain if it has not completed within {@code duration}, regardless of how many retry
   * attempts to remain.
   *
  ...
  /**
   * Registers a synchronous fallback that is invoked once every retry attempt has been exhausted,
   * producing a substitute result instead of failing the chain.
   *
  ...
  /**
   * Same as {@link #onErrorFallback(SynchronousFallbackHandler)}, but the fallback itself produces
  ...
  /**
   * Registers a fallback that resumes the chain with a substitute value whenever a single step
   * fails, without affecting retry accounting.
   *
  ...
  /**
   * Registers a hook invoked right after a step completes successfully, before the next step
   * starts.
   *
   * @param hook the callback to invoke
   * @return the same chain, still producing {@code T}
  ...
  /**
   * Registers a hook invoked whenever a failed step triggers a new retry attempt.
   *
   * @param hook the callback to invoke
   * @return the same chain, still producing {@code T}
   */
  ...
  /**
   * Executes the chain asynchronously, without a timeout unless {@link #withTimeout(Duration)} was
   * configured. Steps that were defined synchronously are dispatched onto a virtual-thread pool.
   *
  ...
  /**
   * Same as {@link #asynchronously()}, but synchronous steps are dispatched onto the given executor
   * instead of the default virtual-thread pool.
   *
  ...
  /**
   * Executes the chain synchronously on the calling thread, without a timeout unless {@link
   * #withTimeout(Duration)} was configured. Steps that were defined asynchronously are joined.
   *
   * @deprecated blocks the calling {@link Thread} until the chain completes — never call this on
  ...
  /**
   * Same as {@link #blocking()}, but fails if the chain has not completed within {@code timeout}.
   *
