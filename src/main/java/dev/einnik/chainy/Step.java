 ...
 * <p>
 * This is the piece that makes {@code then(...)} and
 * {@code thenAsynchronously(...)} produce an identical builder: a step is
 * defined once, independent of how it will later run. {@link #executeBlocking}
 * and {@link #executeAsync} are two different ways to run the very same
 * step — which one is called is decided by the chain's terminal operation
 * ({@code blocking()} / {@code asynchronously()}), never by the step itself.
 *
...
public interface Step<IN, OUT> {
     ...
     */
    @NonNull CompletableFuture<OUT> executeAsync(@NonNull IN input, @NonNull Executor executor) throws CompletionException;
     ...
     */
    static <IN, OUT> @NonNull Step<IN, OUT> ofSync(int order, @NonNull ChainyFunction<IN, OUT> function) {
        return new Step<>() {
            @Override
            public int order() {
                return order;
            ...
            @Override
            public @NonNull CompletableFuture<OUT> executeAsync(@NonNull IN input, @NonNull Executor executor) throws CompletionException {
     ...
     * @return the function wrapped into a step
     */
    static <IN, OUT> @NonNull Step<IN, OUT> ofAsync(int order, @NonNull AsyncChainyFunction<IN, OUT> function) {
        return new Step<>() {
            @Override
            public int order() {
                return order;
     ...
     * @return the function wrapped into a step
     */
    static <IN, OUT> @NonNull Step<IN, OUT> ofStart(int order, @NonNull ChainySupplier<OUT> supplier) {
     ...
     * @return the function wrapped into a step
     */
    static <IN, OUT> @NonNull Step<IN, OUT> ofStartAsync(int order, @NonNull ChainySupplier<CompletableFuture<OUT>> supplier) {
        return ofAsync(order, _ -> {
            try {
                return supplier.get();
            } catch (CompletionException e) {
                return CompletableFuture.failedFuture(e);
    ...
    static <T> @NonNull Step<T, T> ofConditional(
            int order, @NonNull Predicate<T> predicate, @NonNull ChainyFunction<T, T> ifFunction, @NonNull ChainyFunction<T, T> elseFunction) {
        return ofSync(order, value -> predicate.test(value) ? ifFunction.apply(value) : elseFunction.apply(value));
    }
    ...
    static <T> @NonNull Step<T, T> ofDelay(int order, @NonNull Duration duration) {
        return new Step<>() {
            ...
            @Override
            public @NonNull CompletableFuture<T> executeAsync(@NonNull T input, @NonNull Executor executor) throws CompletionException {
                Executor delayed = CompletableFuture.delayedExecutor(duration.toMillis(), TimeUnit.MILLISECONDS, executor);
     ...
     * @return the function wrapped into a step
     */
    static <T, R> @NonNull Step<T, List<R>> ofAll(int order, @NonNull List<AsyncChainyFunction<T, R>> functions) {
        return new Step<>() {
            @Override
            public int order() {
                return order;
            ...
            @Override
            public @NonNull CompletableFuture<List<R>> executeAsync(@NonNull T input, @NonNull Executor executor) throws CompletionException {
                List<CompletableFuture<R>> futures = functions.stream()
                        .map(function -> {
                            try {
                                return function.apply(input);
                            } catch (Exception e) {
